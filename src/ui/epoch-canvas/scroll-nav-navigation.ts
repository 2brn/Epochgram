...
export function navigateToEntryTarget(canvas: EpochCanvas, target: Extract<ScrollNavTarget, { kind: "entry" }>): void {
	...
	try {
		const path = String((target.entry as any)?.file ?? "");
		const line0 = Number((target.entry as any)?.blockStart ?? 0);
		...
		if (path) {
			...
			c.__forcedActiveFileCursorUntil = performance.now() + 15_000;
		}
	} catch {
		// ignore
	}
...
export function hasVisibleEntryForFile(
	...
	for (const layout of c.layouts) {
		...
		for (const summary of layout.summaryRects) {
			const entryFile = (summary as any)?.entry?.file;
			if (entryFile !== filePath) continue;
			if (hasLine) {
				const entry = (summary as any)?.entry;
			...
			const y1 = Number((summary as any)?.y1);
			const y2 = Number((summary as any)?.y2);
