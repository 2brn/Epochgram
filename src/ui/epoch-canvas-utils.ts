...
if (SUMMARY_ICON_ICS_SYNC) SUMMARY_ICON_LABELS[SUMMARY_ICON_ICS_SYNC] = "SYNC";
...
export function entrySummaryComponents(entry: DateEntry): { base: string; iconIds: string[] } {
	...
	switch (entry.source) {
		...
		case "namedate":
			...
			break;
		case "dateprop":
			addIcon(entry.icsSynced ? SUMMARY_ICON_ICS_SYNC : (isMarkdownEntry(entry) ? SUMMARY_ICON_NOTE : SUMMARY_ICON_ATTACHMENT));
			break;
		default:
