...
- Settings UI (Verified):
  ...
  - General settings are grouped into:
    ...
    - `Indexer`: `Track changes` (Pro-gated/blurred when unavailable), `Anchor mdate`, `Anchor property`, `Summary property`, `Parse all properties`, `Filename length`, `Summary length`, `Similarity` group, `Generative AI` group (desktop-only), and `Index` actions.
